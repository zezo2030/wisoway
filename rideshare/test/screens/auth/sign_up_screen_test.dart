...
import 'package:rideshare/screens/auth/sign_up_screen.dart';
...
void main() {
  ...
  group('SignUpScreen hero header', () {
    // The header used to be stacked over the illustration, which left the
    // subtitle sitting on the car and the skyline behind it.
    for (final locale in const [Locale('ar'), Locale('en')]) {
      ...
      testWidgets('keeps the title block clear of the illustration in $code', (
        ...
        for (final above in [
          find.text(l10n.passengerSignupTitle),
          find.text(l10n.passengerSignupSubtitle),
    ...
    testWidgets('keeps the header ink dark under the dark theme', (
      ...
      for (final text in [
        l10n.passengerSignupTitle,
        l10n.passengerSignupSubtitle,
      ]) {
    ...
    testWidgets('moves the numbered stepper into the form sheet', (
      tester,
    ) async {
      await _pump(tester, const Locale('ar'));

      // Below the art, i.e. inside the sheet — the top bar keeps only the back
      // chevron and the "step 1 of 3" chip.
      expect(
        tester.getRect(find.byType(AuthStepIndicator)).top,
...
final Finder _heroArt = find.byWidgetPredicate(
  (w) =>
      ...
      (w.image as AssetImage).assetName.contains('auth_passenger_signup_hero'),
);
  ...
  await tester.pumpWidget(
    MultiProvider(
      ...
      child: MaterialApp(
        ...
        // overflow the hero purely as a metrics artifact.
